...
<!-- Default box -->
    <div class="card">
      <div class="card-header">
        <h3 class="card-title"><strong>Lista de las ordenes</strong></h3>
                    ...
                    <tr id="{{$order->order_number}}">
                      ...
                      <td class="align-middle"><select class="custom-select select-order-state" data-url="{{route('order-change-state')}}" data-id="{{$order->order_id}}">
                        @foreach($order_states as $order_state)
                          @if($order_state->order_state_id == 1 or $order_state->order_state_id == 2)
                         <option value="{{$order_state->order_state_id}}" @if($order_state->order_state_id == $order->order_state_id) selected @endif >@if($order_state->order_state_id == 2) Recibir @else {{ ucwords($order_state->order_state_name) }} @endif </option>
                         @endif
                        @endforeach
                        </select>
                      </td>
...
{{--                         <a href="{{route('order-edit', ['order_id' => $order->order_id])}}" class="btn btn-success" title="Editar"><i class="fas fa-edit"></i></a>
      ...
      <div class="card-footer">
        Hay <strong>{{count($orders)}}</strong> ordenes registradas
      </div>
<!-- /.card-footer-->
  </div>
<!-- /.card -->
